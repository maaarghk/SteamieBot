...
class YouTubeInfo(object):
        ...
        def auth():
                ...
                if credentials is None or credentials.invalid:
                        flow = 
flow_from_clientsecrets("secrets/google_client_secrets.json",scope=scope)
        ...
        def getTitle(self,this_id):
                if not self.validate(this_id):
                        return "Invalid ID"
                response = 
self.yt.videos().list(part="id,snippet,statistics,contentDetails",id=this_id).execute()
